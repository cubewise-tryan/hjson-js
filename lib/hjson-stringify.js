...
module.exports = function(data, opt) {
  ...
  if (opt && typeof opt === 'object') {
    ...
    if (opt.multiline && typeof opt.multiline === "object"){
      multilineForce = true;
  ...
  function quote(string, key, gap, hasComment, isRootObject) {
   if(multilineForce && multiline[key]){
      // Force the a multiline string to be used
      return mlString(string, gap, true);
    } else if (!string) return wrap(token.qstr, '');
  ...
  function mlString(string, gap, force) {
  ...
  function str(value, key, hasComment, noIndent, isRootObject) {
    ...
    switch (typeof value) {
      case 'string':
        return quote(value, key, gap, hasComment, isRootObject);
      ...
      case 'object':
        ...
        if (isArray) {
          ...
          for (i = 0, length = value.length; i < length; i++) {
            ...
            wrapLen = 0;
            v = value[i];
            partial.push(str(v, k, comments ? ca : false, true) + (setsep ? separator : ''));
            if (cpartial) {
              ...
              switch (typeof v) {
                case 'string':
                  ...
                  cpartial.push(str(v, k, false, true) + (setsep ? token.com[0] : ''));
  ...
  res += str(data, null, null, true, true);
